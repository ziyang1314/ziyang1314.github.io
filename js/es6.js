let functionArray = {
  ...
  arr() {
    ...
    console.log(
      "Array.fill(),给数组填充数据，三个参数，1，填充内容，指定起始位置，结束位置",
      arr6
    );
  ...
  obj() {
    ...
    console.log(
      ...
      Object.getOwnPropertySymbols(objExample)
    );
  ...
  sort() {
    ...
    for (let i = 1; i < arr.length; i++) {
      ...
      for (let j = 0; j < arr.length - i; j++) {
        if (arr[j] > arr[j + 1]) {
          // 取数组中的数据从第一个和第二个作对比，第二个比第一个小，就交换位置；这样交换到最后，最后一位就是最大的；
    ...
    console.log(
      `循环一亿次产生的阻塞时间差为:${new Date().getTime() - currenttime}毫秒`
    );
    return arr;
  },
  aboutEval() {
      ...
      return {
        next: function () {
          if (this._first) {
            this._first = false;
            console.log(
              "借助此属性我们可以修改这些数据的返回值--比较对对象的proxy和reflect"
            );
            return {
              value: "bye",
              done: false,
  ...
  deepCopy(copyObject) {
    ...
    var target = {};
    // copyObject 类型确定
    console.log(typeof copyObject);
    // copyObject 存在并且目标是简单类型，直接复制
    if (copyObject && copyObject !== "object") {
      target = copyObject;
    } else if (copyObject && copyObject === "object") {
      // 判断是数组还是对象；
      target = Array.isArray(copyObject) ? [] : {};
      // 遍历对象然后进行赋值；
      for (let key of copyObject) {
        if (copyObject.prototype.hasOwnProperty(key)) {
          if (copyObject[key] && typeof copyObject[key] === "object") {
            target[key] = deepCopy(copyObject[key]);
          } else {
            target[key] = copyObject[key];
          }
        }
      }
    }
